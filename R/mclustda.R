                     ...
                     initialization = NULL, warn = FALSE, ...) 
{
  ...
  names(Models) <- classLabel
  Models$Vinv <- NULL
  out <- list(call = call, type = modelType,
              models = Models, n = n, d = p, 
...
logLik.MclustDA <- function(object, data, ...)
{
  ...
  if(object$type == "EDDA")
    { df <- d*nclass + nVarParams(object$models[[1]]$modelName, d = d, G = nclass) }
  else
    { df <- sum(sapply(object$models, function(mod) 
               with(mod, (G-1) + G*d + nVarParams(modelName, d = d, G = G)))) }
  #

  # this functions substitute exp and rowSums to avoid Inf in calculation below
  Exp <- function(x) 
  { out <- exp(x) 
    out[!is.finite(out)] <- .Machine$double.xmax
    return(out)
  }
  RowSums <- function(x)
  { out <- apply(x, 1, sum)
     ...
     { ll <- matrix(NA, n, G[k])
       for(j in seq(G[k]))
          { if(d > 1) 
              { m <- par[[k]]$mean[,j]
                v <- par[[k]]$variance[,,j] 
                cdens <- dmvnorm(data, m, v, log = TRUE)
              }
            else
              { m <- par[[k]]$mean[j]
                v <- par[[k]]$variance[j] 
                cdens <- dnorm(data, m, v, log = TRUE)
              }
            ll[,j] <- log(par[[k]]$pro[j]) + cdens
          }
       l[,k] <- RowSums(Exp(ll))
     }
  l <- sum(log(apply(l,1,function(l)sum(fclass*l))))

  # l <- sum(log(do.call("dens", c(list(data = data), mod))))
  # ll <- sapply(object$models, function(mod) 
  #       { do.call("dens", c(list(data = data), mod)) })
  # sum(log(apply(ll,1,function(l)sum(fclass*l))))
...
plot.MclustDA <- function(x, what = c("scatterplot", "classification", "train&test", "error"), newdata, newclass, dimens, symbols, colors, ...)
{
  ...
  trainClass <- eval.parent(object$call$class)
  ...
  if(what == "scatterplot")
    {
      if(length(dimens) == 1)
        { eval.points <- seq(min(data[,dimens]), max(data[,dimens]), length = 1000)
          d <- matrix(NA, length(eval.points), nclass)
          for(i in seq(nclass))
...
# if nfold=3 returns 2:1 CV error
  ...
  call <- object$call
  data <- eval.parent(call$data)
  class <- as.factor(eval.parent(call$class))
  n <- length(class)
